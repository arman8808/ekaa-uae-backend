...
const isValidEventPayload = ({ event, level }) => {
  ...
  if (def.hasLevels) {
    if (!level) return { ok: false, message: "Level is required for this event" };
    if (!def.levels.includes(level)) return { ok: false, message: "Invalid level for selected event" };
  }
  return { ok: true };
};
...
exports.createManagedEvent = async (req, res) => {
  try {
    const { event, level, startDate, endDate, status, location, conductedBy, totalParticipants, programFees } = req.body;
...
exports.getManagedEvents = async (req, res) => {
  try {
    ...
    const data = await ManagedEvent.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit);
...
exports.getManagedEventById = async (req, res) => {
  try {
    const doc = await ManagedEvent.findOne({ _id: req.params.id, isDeleted: false });
    if (!doc) {
...
exports.updateManagedEvent = async (req, res) => {
  try {
    ...
    if (event || level) {
      const validation = isValidEventPayload({ event: event || req.body.event, level });
      if (!validation.ok) {
        return res.status(400).json({ success: false, message: validation.message });
      }
    }
    ...
    if (totalParticipants !== undefined) updates.totalParticipants = totalParticipants;
